    ...
    <div class="container w3-content" style="max-width:1400px;margin-top:20px">
      ...
      <div class="w3-row">
        ...
        if(isset($_POST['cari'])){ //jika search dilakukan maka menjalankan query bagian ini
			...
			$sql = "select lapangan.*, operator.alamat, operator.kota, operator.nama_lapangan from lapangan inner join operator on (lapangan.username=operator.username) where nama_lapangan like '%$cari%' limit $posisi, $batas"; //query memilih data dari tabel lapangan dan operator yang memiliki nama tempat tenis mirip dengan yang dicari 
			}else{ // jika form cari kosong
				$sql = "select lapangan.*, operator.alamat, operator.kota, operator.nama_lapangan from lapangan inner join operator on (lapangan.username=operator.username) where nama_lapangan like '%%' limit $posisi, $batas"; //query memilih data dari tabel lapangan dan operator yang memiliki nama tempat tenis mirip dengan yang dicari
				}
			...
			$sql = "select lapangan.*, operator.alamat, operator.kota, operator.nama_lapangan from lapangan inner join operator on (lapangan.username=operator.username) where (kota like '%$lokasi%') and (harga between '$pecah[0]' and '$pecah[1]') and (jenis_rumput like '%$jenis%') limit $posisi, $batas"; 
			...
			$sql = "select lapangan.*, operator.alamat, operator.kota, operator.nama_lapangan from lapangan inner join operator on (lapangan.username=operator.username) limit $posisi, $batas";	
          ...
          <div class="w3-container w3-card-2 w3-white w3-round" style="margin-left: 10px; margin-top:10px;"><br>
              ...
              <div class="w3-row-padding" style="margin:0 -16px">
                ...
                <div class="w3-half">
                  ...
                  <i class="fa fa-location-arrow" aria-hidden="true"></i>&nbsp;<?php echo $r['alamat']; ?><br><br>
